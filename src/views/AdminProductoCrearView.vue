...
import { getCategories, getMarcas, createProduct } from '../services/api'
...
async function crear(){
  ...
  try{
    ...
    for(const k in form.value){ if(form.value[k]!=='' && form.value[k]!==null) fd.append(k, form.value[k]) }
    if(archivo.value) fd.append('archivo', archivo.value)
    const res = await createProduct(fd)
    exito.value = res.message || 'Producto creado'
    setTimeout(()=> router.push('/admin/productos'), 900)
  }catch(err){ error.value = err.response?.data?.message || 'Error al crear' } finally { guardando.value=false }
}
onMounted(async ()=>{
